...
const scaleY = scaleLinear().domain([0, maxY]).range([height - padding, padding]);
const scaleHeight = scaleLinear().domain([0, maxY]).range([0, height - 2 * padding]);
    ...
    return (
        <View style={{backgroundColor: '#fff'}}>
            <Svg width={width} height={height}>
                ...
                {
                    // horizontal lines
                    generateYAxisValues(stepSize, 0, maxY).map((v, index) => (
                        <Path stroke='#adadad'
